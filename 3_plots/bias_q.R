...
coef       <- cbind(scen.mat, bias=rep(NA,length(list.out)))                    # save the conditions and the output
for(i in 1:length(list.out)){
  coef[i,5] <- list.out[[i]][2,7]  
}
...
coef1      <- cbind(scen.mat, bias=rep(NA,length(list.out)))                    # save the conditions and the output
for(i in 1:length(list.out)){
  coef1[i,5] <- list.out[[i]][2,7]  
}
...
coef3      <- cbind(scen.mat, bias=rep(NA,length(list.out)))                    # save the conditions and the output
for(i in 1:length(list.out)){
  coef3[i,5] <- list.out[[i]][2,7]  
}
...
coefBCH     <- cbind(scen.mat, bias=rep(NA,length(list.out)))                   # save the conditions and the output
for(i in 1:length(list.out)){
  coefBCH[i,5] <- list.out[[i]][2,7]  
}
...
ggplot(tabel, aes(x=coml, y=bias, group=models)) +                              # create a ggplot
