...
scen.mat <- expand.grid(scens)                                                  # list for output
...
coef <- cbind(scen.mat, sesd=rep(NA,length(list.out)))                          # save the conditions and the output
for(i in 1:length(list.out)){
  coef[i,5] <- list.out[[i]][2,9]  
}
...
coef1 <- cbind(scen.mat, sesd=rep(NA,length(list.out)))                         # save the conditions and the output
for(i in 1:length(list.out)){
  coef1[i,5] <- list.out[[i]][2,9]  
}
...
coef3 <- cbind(scen.mat, sesd=rep(NA,length(list.out)))                         # save the conditions and the output
for(i in 1:length(list.out)){
  coef3[i,5] <- list.out[[i]][2,9]  
}
...
coefBCH <- cbind(scen.mat, sesd=rep(NA,length(list.out)))                       # save the conditions and the output
for(i in 1:length(list.out)){
  coefBCH[i,5] <- list.out[[i]][2,9]  
}
...
ggplot(tabel, aes(x=coml, y=sesd, group=models)) +                              # create a ggplot
