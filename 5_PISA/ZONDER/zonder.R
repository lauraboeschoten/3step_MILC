...
data  <- merge(data1, data2, by="CNTSTUID")

setwd("F:datapackage_threestepMILC/5_PISA/excluded")

gdata <- NULL    

for (i in 1:ncol(data)){                                                        
  gdata <- cbind(gdata,gsub(" ","",data[,i]))                                   # remove spaces
}

gdata        <- data.frame(gdata)
names(gdata) <- names(data)

short_data   <- ddply(gdata,.(CM033Q01S,CM474Q01S,CM155Q01S,CM155Q04S,CM411Q01S,# create short dataset
...
boots           <- rmultinom(5, nrow(gdata), short_data$V1/sum(short_data$V1))  # create 5 bootstraps    
...
outfiles           <- paste0("3_model.",1:5)                                    # run LC models
bootstrap.colnames <- tail(colnames(bdata),5)

envir <- new.env()                                
run.template(template.path="2_LC1.brew", envir=envir,                           
             temp.filename.base=outfiles[1],                                    
             bootstrap.colnames=bootstrap.colnames[1])         

envir <- new.env()                                                              
run.template(template.path="2_LC2.brew", envir=envir,                           
             temp.filename.base=outfiles[2],                                    
             bootstrap.colnames=bootstrap.colnames[2]) 

envir <- new.env()                                                              
run.template(template.path="2_LC3.brew", envir=envir,                           
             temp.filename.base=outfiles[3],                                    
             bootstrap.colnames=bootstrap.colnames[3])

envir <- new.env()                                                              
run.template(template.path="2_LC4.brew", envir=envir,                           
             temp.filename.base=outfiles[4],                                    
             bootstrap.colnames=bootstrap.colnames[4]) 

envir <- new.env()                                                              
run.template(template.path="2_LC5.brew", envir=envir,                           
             temp.filename.base=outfiles[5],                                    
             bootstrap.colnames=bootstrap.colnames[5])  

################################################################################

scan.mod        <- readLines("3_model.1.lst")                                   # load LC model
lin.size        <- grep('Size',scan.mod)                                        # overall cluster sizes 
...
vars <- unlist(strsplit(scan.mod[a:(b-1)], " "))[c(6,8,10,12,14,16,32,34,36,38, # variable names
                                                   40,42,58,60,62,64,66,68,84,86)]

pr             <- grep('Profile',scan.mod)                                      # profiles
matlist        <- vector("list", length(vars))                                 
names(matlist) <- vars                                                          

for (i in 1:length(vars)){
  c <- grep(vars[i],scan.mod)[[which(grep(vars[i],scan.mod)>pr)[1]]]            # place of variable name in profile
  if(i!=length(vars)){
    d <- grep(vars[i+1],scan.mod)[[which(grep(vars[i+1],scan.mod)>pr)[1]]]      # next variable name
  } else {d <- grep('ProbMeans-Posterior',scan.mod)}                            # put whats inbetween in a matrix
  matlist[[i]] <- matrix(unlist(strsplit(scan.mod[(c+1):(d-1)], "\t")),         # make list of matrices
                         ncol=7, byrow=T)[,c(1,2,4)]
}

varsco <- unlist(strsplit(scan.mod[b+1], " "))[c(14,16)]                        # covariates
matco        <- vector("list", length(varsco))                                 
names(matco) <- varsco
b2 <- grep('Independent',scan.mod)[[2]]  

for (i in 1:length(varsco)){                                                    # same for covariate in probmeans
...
for(i in 1:length(matco)){                                                      # turn into conditionals
  for(j in 2:3){                                                                
...
for(i in 1:length(matlist)){                                                    # create "NA's"
  matlist[[i]][matlist[[i]]=="NA"] <- NA                                        
}

ar <- array(NA, dim=c(nrow(bdata),length(matlist),2))                           # empty array

for(j in 1:nrow(bdata)){                                                        # add conditionals
  for(i in 1:length(matlist)){                                                  # turn into numeric
    kkk <- bdata[j,i]                                                           # if NA -> turn into 1
...
for(j in 1:nrow(post)){                                                         # make posteriors
  for (i in 1:2){                                                           
...
for (i in 1:nrow(pdata1)){                                                      
  pdata1[i,"imp"] <- which(rmultinom(1,1,pdata1[i,c("1","2")])==1)              # sample from posteriors
}

write.table(pdata1, "pdata1.txt", quote=FALSE, sep=" ", row.names=FALSE)
pdata1      <- read.table("pdata1.txt", header=TRUE)
pdata1$imp  <- as.factor(pdata1$imp) #X1
...
a    <- grep('dependent',scan.mod)[[2]]                                          
b    <- grep('independent',scan.mod)[[1]]                                        
vars <- unlist(strsplit(scan.mod[a:(b-1)], " "))[c(6,8,10,12,14,16,32,34,36,38,
                                                   40,42,58,60,62,64,66,68,84,86)]
...
pdata2     <- read.table("pdata2.txt", header=TRUE) #X2
...
for (i in 1:nrow(pdata3)){                                                     
  pdata3[i,"imp"] <- which(rmultinom(1,1,pdata3[i,c("1","2")])==1)        
}
...
pdata4$imp <- as.factor(pdata4$imp) #X2
